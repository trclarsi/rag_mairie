...
DIRECTIVES :
1. Réponds UNIQUEMENT via le CONTEXTE DOCUMENTAIRE fourni.
2. Si l'info est absente : "Je ne dispose pas de cette information, veuillez contacter le secrétariat."
3. Ton : Professionnel et courtois.
4. Structure : Utilise des listes à puces si nécessaire.
...
class RAGAgent:
    ...
    def _generate(self, prompt: str, temp=0.2) -> str:
        try:
    ...
    def ask(self, query: str, history: List[Dict] = None, k=3, temperature=0.2) -> Tuple[str, List[Dict]]:
        history = history or []
        ...
        # 3. Génération
        context_text = "\n".join([f"Source {d['source']}: {d['content']}" for d in docs]) if docs else "Aucun document trouvé."
        final_prompt = SYSTEM_PROMPT.format(history=history_text, context=context_text, query=query)
        
        answer = self._generate(final_prompt, temp=temperature)
        return answer, docs
...
def main():
    ...
    print("\n" + "="*60)
    print("🤖 AGENT D'ACCUEIL MAIRIE (RAG GEMINI OPTIMISÉ)")
    print("   -> Mode : Questions/Réponses interactif")
    print("   -> Tapez 'exit', 'quit' ou 'q' pour quitter.")
    print("="*60)
    
    # Historique local pour la session CLI
    cli_history = []
